...
1. 先用 Project 1 在桌面端手动构建一个简单 C 工程。学生能直接运行程序，反馈快，适合讲预处理、编译、汇编、链接、头文件搜索、静态库和常见错误。
...
4. Project 2 先手动构建，再迁移到 EIDE。通过“手动命令 -> EIDE 配置”的对应关系，让学生理解自动构建系统本质上是在保存和组织构建参数。
...
| 25-65 min | 桌面端 C 手动构建 | Project 1 | 预处理、编译、汇编、链接 |
| 65-90 min | 多文件 C 工程和构建配置 | Project 1 | include path、object file、static library、link errors |
...
| 135-170 min | 迁移到 `arm-none-eabi-gcc` | Project 2 | 交叉编译、target flags、`.elf`、`.bin` |
| 170-195 min | startup、linker script、嵌入式启动流程 | Project 2 | Flash/RAM、Reset_Handler、vector table |
| 195-215 min | Project 2 改成 EIDE 项目 | Project 2 | 自动构建系统与手动命令的对应关系 |
...
用桌面端 C 程序讲清楚基本 C 构建流程和规则。这个项目全程手动构建，不使用 Makefile、CMake 或 EIDE，让学生直接看到每个命令在做什么。
...
gcc build/main.o build/led.o -o build/app
./build/app
```
...
- `.h` 主要放声明、类型定义、宏和接口。
- `.c` 放实现。
- `#include "led.h"` 只让编译器看见声明，不会自动链接 `led.c`。
- `-Iinclude` 是头文件搜索路径。
- 链接阶段才会把 `main.o` 和 `led.o` 合起来。
...
git commit -m "Split LED simulation into module"
```
...
Add first desktop C program
Split LED simulation into module
...
把 Project 1 的简单 C 工程迁移到 `arm-none-eabi-gcc`，展示桌面端程序和嵌入式固件的区别。

这个项目分两阶段：

1. 先全程手动构建，理解交叉编译、target flags、startup code、linker script、`.elf`、`.bin`。
2. 再迁移为 EIDE 项目，理解自动构建系统如何保存和组织这些手动命令。
...
建议指定一个真实目标芯片或内核，例如后续课程实际使用的 STM32 型号。如果暂时不定，可以先用 Cortex-M3/M4 作为讲解目标，但最终 slides 里最好替换成真实开发板对应型号。

### 2.1 从桌面端目标迁移到 ARM 目标
...
- `-mcpu=cortex-m3` 指定目标 CPU。
- `-mthumb` 指定 Thumb 指令集。
- `-ffreestanding` 表示不是普通桌面 hosted C 环境。
- 生成的 `.o` 不能直接在电脑上运行。
...
```bash
git init
git add README.md .gitignore include/led.h src/main.c src/led.c
...
### 2.3 startup code 和 vector table

给一个最小 startup 模板，不要求学生完整背写。
...
- 真实 STM32 startup 文件比这个复杂，会处理 `.data`、`.bss`、中断向量。
- `main` 不是 MCU 真正执行的第一段用户相关代码。
- startup 文件通常由芯片厂商、CubeMX 或模板工程提供。

### 2.4 linker script 和内存布局

给一个教学用 linker script。
...
arm-none-eabi-gcc \
  ...
  -nostdlib \
  -T linker/memory.ld \
  build/startup.o build/main.o build/led.o \
...
- `.bin` 是原始二进制内容。
- `.hex` 带地址信息，常用于烧录工具。
- `-nostdlib` 表示不自动链接标准库和默认启动文件。

Git 操作：

```bash
git add src/startup.c linker/memory.ld
...
### 2.6 Project 2 迁移为 EIDE 项目

先展示手动命令，再展示 EIDE 配置如何对应这些命令。

| 手动命令或参数 | EIDE 中对应配置 |
| --- | --- |
| `src/main.c`, `src/led.c`, `src/startup.c` | source files |
| `-Iinclude` | include directories |
| `-DDEBUG` | preprocessor definitions |
| `-mcpu=cortex-m3` | target CPU / compiler options |
| `-mthumb` | compiler options |
| `-T linker/memory.ld` | linker script |
...
需要提醒学生：如果 EIDE 生成了本地缓存或构建目录，要加入 `.gitignore`，不要把构建产物一起提交。
